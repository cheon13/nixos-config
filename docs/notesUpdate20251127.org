#+TITLE: Notes de mise à jour — 27 novembre 2025
#+OPTIONS: toc:nil num:nil

En résumé : l'évaluation échoue sur =noto-fonts-emoji=, renommé en
=noto-fonts-color-emoji=. Trois avertissements l'accompagnent — les options
~programs.git.userName~/~userEmail~/~extraConfig~ renommées vers
~programs.git.settings~, un écart de version entre Home Manager 26.05 et
Nixpkgs 25.11, et un rappel sur la montée de version de Nextcloud.

Session lancée par =./build.sh 'changer firefox-wayland à firefox'=.
